...
namespace CodeBinder.Apple
{
    static partial class ObjCExtensions
    {
        public static bool HasReplacementWriters(this StatementSyntax statement,
            ObjCCompilationContext context, [NotNullWhen(true)]out IEnumerable<CodeWriter>? writers)
        {
        ...
        static bool hasReplacementWriters(ReturnStatementSyntax statement, ObjCCompilationContext context,
        ...
        static bool hasReplacementWriters(ExpressionStatementSyntax statement, ObjCCompilationContext context,
          [NotNullWhen(true)]out IEnumerable<CodeWriter>? writers)
        {
            switch (statement.Expression.ExpressionKind())
            {
        ...
        static bool hasReplacementWriters(LocalDeclarationStatementSyntax statement, ObjCCompilationContext context,
          [NotNullWhen(true)]out IEnumerable<CodeWriter>? writers)
        {
        ...
        static bool hasReplacementWriters(InvocationExpressionSyntax invocation, StatementSyntax statement,
            [NotNullWhen(true)]ObjCCompilationContext context, out IEnumerable<CodeWriter>? writers)
        {
        ...
        static IEnumerable<CodeWriter> getReplacementWriters(InvocationExpressionSyntax invocation,
            StatementSyntax statement, List<RefArgument> refArguments, ObjCCompilationContext context)
        {
            ...
            foreach (var arg in refArguments)
            {
                ...
                if (arg.Type.TypeKind == TypeKind.Enum)
                    boxType = "IntegerBox";
                ////else
                ////    boxType = ObjCUtils.GetRefBoxType(arg.Type.GetFullName());
        ...
        static List<RefArgument> getRefArguments(InvocationExpressionSyntax invocation, ObjCCompilationContext context)
        {
